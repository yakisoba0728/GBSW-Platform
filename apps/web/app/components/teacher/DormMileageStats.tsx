...
export default function DormMileageStats() {
  ...
  const [response, setResponse] =
    useState<DormMileageOverviewResponse>(EMPTY_RESPONSE)
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/teacher/dorm-mileage/analytics/overview?${params.toString()}`,
        {
          signal: ctrl.signal,
          cache: 'no-store',
        },
      ...
      setResponse({
        ...
        categoryStats: Array.isArray(data?.categoryStats)
          ? (data.categoryStats as DormCategoryStat[])
          : [],
        topRules: Array.isArray(data?.topRules)
          ? (data.topRules as DormTopRuleStat[])
          : [],
      })
    } catch (error) {
      if (abortRef.current !== ctrl || ctrl.signal.aborted) {
