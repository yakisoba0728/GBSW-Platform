...
export default function DormEditEntryModal({
  ...
  async function handleSubmit() {
    ...
    const payload: UpdateDormMileageEntryPayload = { reason }

    try {
      const response = await fetch(`/api/teacher/dorm-mileage/entries/${item.id}`, {
        method: 'PATCH',
      ...
      if (!response.ok) {
        setError(result?.message ?? '기숙사 상벌점 내역을 수정하지 못했습니다.')
        return
      }

      await onSaved(result?.message ?? '기숙사 상벌점 내역이 수정되었습니다.')
    } catch {
      setError('기숙사 상벌점 내역 수정 중 문제가 발생했습니다.')
    } finally {
          ...
          <div>
            ...
            {item && (
              <p className="mt-0.5 text-xs" style={{ fontFamily: 'var(--font-noto-sans-kr), sans-serif', color: 'var(--fg-muted)' }}>
                {item.studentName} · {item.grade ? `${item.grade}학년 ` : ''}{item.classNumber}반 {item.studentNumber}번
              </p>
            )}
          </div>
