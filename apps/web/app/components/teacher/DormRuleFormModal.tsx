...
import type { DormMileageRuleSummary, MileageType } from './dorm-mileage-types'
...
export default function DormRuleFormModal({
  open,
  onClose,
  mode,
  rule,
  ...
  useEffect(() => {
    ...
    if (mode === 'edit' && rule) {
      ...
      setUseRange(rule.minScore !== null && rule.maxScore !== null)
      setMinScore(rule.minScore !== null ? String(rule.minScore) : '')
      setMaxScore(rule.maxScore !== null ? String(rule.maxScore) : '')
    } else {
      setType('reward')
      setCategory('')
      setName('')
  ...
  const datalistId = 'dorm-rule-category-list'
    ...
    >
      <div className="space-y-4">
        <div>
          ...
          <select
            value={type}
            onChange={(event) => setType(event.target.value as MileageType)}
