...
import type { MileageType, DormMileageRuleSummary } from './dorm-mileage-types'
...
export default function DormMileageRules({
  ...
  const [editingRule, setEditingRule] = useState<DormMileageRuleSummary | null>(null)
  ...
  function handleEdit(rule: DormMileageRuleSummary) {
    setFormMode('edit')
    setEditingRule(rule)
    setFormModalOpen(true)
  }
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      ...
      <Card>
        <SectionHeader
          title="기숙사 상벌점 항목"
          subtitle={readOnly ? '현재 등록된 기숙사 상벌점 규칙 목록입니다.' : '현재 등록된 기숙사 상벌점 규칙을 관리합니다.'}
          action={
            readOnly ? undefined : (
