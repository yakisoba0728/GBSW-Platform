...
import type { SharedMileageStudentOption } from './shared-mileage-types'

type SharedStudentSelectionModalProps<Student extends SharedMileageStudentOption> = {
  isOpen: boolean
...
export default function SharedStudentSelectionModal<Student extends SharedMileageStudentOption>({
  isOpen,
  addedStudentIds,
  onClose,
  onConfirm,
...
}: SharedStudentSelectionModalProps<Student>) {
  const [school, setSchool] = useState('')
  const [grade, setGrade] = useState('')
  const [classNumber, setClassNumber] = useState('')
  const [name, setName] = useState('')
  const [students, setStudents] = useState<Student[]>([])
  const [selectedStudents, setSelectedStudents] = useState<Map<string, Student>>(
    new Map(),
  )
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (showSchoolFilter && school) {
        params.set('school', school)
      }
      if (grade) {
        params.set('year', grade)
      }
      if (classNumber) {
  ...
  const filterClassName = showSchoolFilter
    ? 'grid grid-cols-2 gap-2 px-5 py-4 md:grid-cols-[1fr_100px_100px_1fr]'
    : 'grid grid-cols-2 gap-2 px-5 py-4 md:grid-cols-3'
  const searchInputClassName = showSchoolFilter
    ? 'col-span-2 rounded-lg border px-3 py-2 text-sm outline-none md:col-span-1'
    : 'col-span-2 rounded-lg border px-3 py-2 text-sm outline-none md:col-span-1'
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {showSchoolLabel && student.school && (
                        <p
                          className="mt-0.5 text-xs"
                          style={{
                            fontFamily: 'var(--font-noto-sans-kr), sans-serif',
