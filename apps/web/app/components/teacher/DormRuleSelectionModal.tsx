...
export default function DormRuleSelectionModal({
  isOpen,
  rewardRules,
  penaltyRules,
  currentRuleId,
  ...
  function hasScoreRange(rule: DormMileageRuleSummary) {
    return rule.minScore !== null && rule.maxScore !== null
  }

  function getScoreForRule(rule: DormMileageRuleSummary): number {
  ...
  function handleSelectRule(rule: DormMileageRuleSummary) {
    const score = getScoreForRule(rule)
    onSelect(rule, score)
    handleClose()
  }

  function renderRuleItem(rule: DormMileageRuleSummary) {
      ...
      >
        ...
        >
          <div>
            ...
            >
              항목을 선택하면 기본 점수가 자동 적용됩니다. 범위가 있는 항목은 점수를 직접 입력할 수 있습니다.
            </p>
          </div>
          <button
            type="button"
