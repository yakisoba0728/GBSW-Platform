...
import type { DormMileageRuleSummary } from './student-dorm-mileage-types'
...
function formatScoreRange(rule: DormMileageRuleSummary): string {
...
export default function StudentDormMileageRules() {
  ...
  useEffect(() => {
    ...
    async function fetchRules() {
      ...
      try {
        const response = await fetch('/api/student/dorm-mileage/rules', {
          cache: 'no-store',
        ...
        if (!response.ok) {
          setError(result?.message ?? '기숙사 규정 목록을 불러오지 못했습니다.')
          return
        }

        setRules(
          Array.isArray(result?.rules) ? (result.rules as DormMileageRuleSummary[]) : [],
        )
      } catch {
        if (abortController.signal.aborted) return
        setError('기숙사 규정 목록 조회 중 문제가 발생했습니다.')
      } finally {
  ...
  const grouped = useMemo(() => {
    ...
    const map = new Map<string, DormMileageRuleSummary[]>()
          ...
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    layoutId="dorm-rule-tab-indicator"
