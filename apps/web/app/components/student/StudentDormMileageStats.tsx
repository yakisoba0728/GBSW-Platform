...
import { Card, StatCard, ScoreSummaryBar } from '@/app/components/mileage/shared'
...
export default function StudentDormMileageStats() {
  ...
  const [summary, setSummary] = useState<StudentDormMileageSummary | null>(null)
  const [entries, setEntries] = useState<StudentDormMileageHistoryItem[]>([])
    ...
    async function fetchData() {
      ...
      try {
        const [summaryRes, allEntries] = await Promise.all([
          ...
          fetchAllDormEntries(controller.signal),
        ])
...
async function fetchAllDormEntries(signal: AbortSignal) {
  const firstPage = await fetchDormEntriesPage(1, signal)
  ...
  const remainingPages = await Promise.all(
    Array.from({ length: totalPages - 1 }, (_, index) =>
      fetchDormEntriesPage(index + 2, signal),
    ),
  )
...
async function fetchDormEntriesPage(page: number, signal: AbortSignal) {
  const response = await fetch(
    `/api/student/dorm-mileage/entries?page=${page}&pageSize=${ENTRY_PAGE_SIZE}`,
    {
      cache: 'no-store',
      signal,
    },
  ...
  if (!response.ok) {
    throw new Error(result?.message ?? '기숙사 상벌점 내역을 불러오지 못했습니다.')
  }
